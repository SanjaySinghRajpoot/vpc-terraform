resource "aws_instance" "web" {
  ...
  // User data will be ran when a new EC2 instance is created 
  // This is done to make the EC2 setup more presentable
  user_data = <<EOF
  #!/bin/bash
    sudo yum update -y
    ...
    echo "        }" >> index.html
    echo "    </style>" >> index.html
    echo "    <title>Amazon EC2 Application status</title>" >> index.html
    echo "</head>" >> index.html
    echo "<body>" >> index.html
